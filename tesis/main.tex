...
\begin{flushright}
	\textit{El que tenga miedo a morir \\ que no nazca}\\[0.5cm]
	Anónimo
\end{flushright}
...
\textbf{``Búsqueda de agrupaciones en data proveniente de electrocardiogramas (ECG), mediante el Análisis de Componentes Principales (PCA) y el uso de Redes Neuronales.}\\[0.6cm]

Trabajo especial de grado como requisito para optar al Título de Licenciado en Física.
\end{center}
...
\hfill
\begin{flushright}
	\textbf{Autor:}  Br. Abrahan David Quintero Teran\\
	\textbf{Tutor:} Prof. Juan Villegas\\
	Mérida-Venezuela, enero de 2026
\end{flushright}
...
\noindent
Las enfermedades cardiovasculares son la principal causa de muerte entre hombres y mujeres en el mundo, con alrededor de 17.9 millones personas que mueren al año a causa de estas. Los errores provenientes de la interpretación manual de los electrocardiogramas y la variabilidad interobservador de los diagnósticos contribuyen a esta mortalidad. Es por esto que surge la necesidad de presentar un sistema que permita usar eficientemente en términos de cómputo, la información inherente de los electrocardiogramas, empleando una cadena de trabajo que presenta en sus eslabones técnicas como el análisis de componentes principales y la incrustación de vecinos estocásticos como extractores de características junto con redes neuronales perceptrónicas para la identificación de variables clínicas de relevancia, como lo pueden ser el tipo de trastorno cardiovascular presente, grupos etarios o sexo de cada paciente, logrando exactitudes superiores al 85\% para las tareas de identificación previamente mencionadas, reduciendo el costo computacional de forma considerable. Además se propone el uso del electrocardiograma como identificador biométrico y usando el mismo flujo de trabajo antes mencionado, se obtienen exactitudes superiores al 95\% lo que se compara con modelos propuestos en la literatura que aplican técnicas más complejas y costosas en términos económicos y computacionales.

\vspace{0.8cm}

\noindent\textbf{Palabras clave:} Electrocardiograma, derivación II, análisis de componentes principales (PCA), redes neuronales perceptrónicas (NN), matriz de confusión, incrustación de vecinos estocásticos distribuidos en t (t-SNE), dendrograma, distancia de mahalanobis.
\chapter*{Introducción}
%{\huge\textbf{Introducción}}\\


La presente tesis propone y evalúa una cadena de procesamiento para el análisis de señales electrocardiográficas orientada tanto al reconocimiento biométrico como a la clasificación según variables clínicas de interés. Partiendo de la derivación II de los registros electrocardiográficos extraídos de diversas bases de datos de Physionet, se diseñó un flujo metodológico que integra etapas de preprocesamiento, reducción de dimensionalidad y modelado supervisado, con el objetivo de extraer y explotar de forma eficiente las características discriminativas contenidas en las señales cardíacas. En particular, se incorpora la detección de complejos QRS mediante el algoritmo de Pan \& Tompkins, junto con la descomposición vibracional de Hilbert para la corrección de la línea de base, a fin de obtener registros depurados que minimicen el impacto de ruido y artefactos sobre las etapas posteriores de análisis. \\

Sobre estas señales preprocesadas, se emplean técnicas de reducción de dimensionalidad lineal (PCA) y no lineal (t-SNE) para proyectar las representaciones electrocardiográficas hacia espacios de menor dimensión que preserven, en la medida de los posible, la estructura relevante de los datos. A partir de dichas representaciones, se entrenan redes neuronales perceptrónicas para abordar distintas tareas: identificación biométrica de pacientes, discriminación entre estados de salud (sanos vs patológicos), clasificación por grupos etarios, clasificación por sexo y diferenciación entre tipos específicos de trastornos clínicos. Adicionalmente, se utilizan herramientas como matrices de confusión, histogramas de precisión y dendrogramas basados en distancias de Mahalanobis para explorar la estructura latente de los datos y evaluar la separabilidad entre clases desde una perspectiva tanto cuantitativa como visual. \\

Finalmente, la metodología propuesta se valida externamente sobre la base de datos PTB-ECG, replicando esquemas de partición de datos reportados en la literatura (70\% entrenamiento, 30\% validación) con el fin de comparar de manera directa el desempeño y el costo computacional frente a enfoques previos. Los resultados obtenidos permiten analizar hasta qué punto una cadena de procesamiento basada en modelos relativamente simples, pero apoyando en una extracción de características adecuada, puede alcanzar niveles de exactitud cercanos o superiores a los de arquitecturas más complejas, reduciendo de forma significativa los requerimientos computacionales. 
...
\section{Objetivos}
	...
	\subsection{Objetivo General}
		Analizar los datos provenientes de electrocardiogramas mediante la aplicación de análisis de componentes principales (PCA) y redes neuronales, con la finalidad de identificar agrupaciones en dicha data.
...
\begin{itemize}
	...
	\item El método propuesto es capaz de desempeñarse mejor bajo condiciones de severa distorsión de la línea base sin afectar la morfología real del ECG.
\end{itemize}
...
\begin{itemize}
	...
	\item Todos estos modelos fueron entrenados con el mismo numero de muestras y configuración de hiperparámetros.
	\item Evalúan el desempeño de cada modelo, utilizando métricas comunes como exactitud (accuracy), recall, F1-score y precisión.
\end{itemize}
...
\subsection{Análisis de Componentes Principales (PCA).} \label{subsec:PCA}
	...
	\begin{enumerate}
		...
		Las diferentes características que están presentes en el conjunto de datos pueden tener diferentes unidades y escalas. Para comparar estas equitativamente, primero se debe estandarizar los datos haciendo que cada característica tenga: 
		\begin{itemize}
			\item Una media de $0$.
...
%	Estos modelos pueden utilizarse como método alternativo en análisis y predicciones. Al estudio de estas redes se le conoce como Aprendizaje Profundo y su uso esta siendo elevado hoy en día en numerosas aplicaciones, desde manejo autónomo de vehículos hasta en aplicaciones de imágenes medicas. Su funcionamiento es del tipo «caja negra», es decir, estos modelos no requieren información detallada del sistema ya que aprenden de la relación entre los parámetros de entrada, similar a una regresión no lineal, pero con mucha más potencia que una de estas, manejando sistemas grandes y complejos con muchos parámetros interrelacionados.
	...
	\begin{itemize}
		...
		\item \textbf{Regla de propagación:} Se realizan diferentes operaciones con las entradas y los pesos. Con esto se busca obtener el valor del potencial que posteriormente se utilizará para realizar el procesamiento. Una de las operaciones más comunes es la suma ponderada. Otra regla puede ser la distancia euclidiana, que es utilizada por los mapas auto-organizados y las redes de función de base radial. 
	...
	Al combinar varios perceptrones dispuestos en forma de capas, se puede generalizar al perceptrón simple, este surgió como consecuencia de las limitaciones del mismo en lo referente al problema de separabilidad no lineal. Minsky y Papert mostraron en 1969 que la combinación de varios perceptrones simples podía resultar en una solución adecuada para tratar ciertos problemas no lineales. Sin embargo, los autores no presentaron una solución al problema de como adaptar los pesos de la capa de entrada a la capa oculta, pues la regla de aprendizaje del perceptrón simple no puede aplicarse a este escenario. No obstante, la idea de combinar varios perceptrones sirvió de base para estudios posteriores realizados por Rumelhart, Hinton y Williams en 1986 \cite{percepMulti}. Estos autores mostraron que una retropropagación de los errores medidos en la salida de la red hacia los perceptrones de las capas ocultas, dando lugar a la llamada regla delta generalizada. Diferentes autores han demostrado independientemente que el perceptrón multicapa es un aproximador universal, en el sentido de que cualquier función continua en un espacio $R^n$ puede aproximarse con un perceptrón multicapa, con al menos una capa oculta de neuronas \cite{stephani}.
 	...
 	\begin{itemize}
 		...
 		\begin{equation}
 			\frac{\partial e_p}{\partial Y_{p j}}=\frac{\partial \frac{1}{2} \sum_{j=1}^M\left(D_{p j}-Y_{p j}\right)^2}{\partial Y_{p j}}=-\left(D_{p j}-Y_{p j}\right)
 		\end{equation}
...
\begin{itemize}
	...
	\item La base de datos PTB diagnostica (PTB Diagnostic ECG Database) es una colección de electrocardiogramas que fueron obtenidos usando un prototipo no comercial, esta base de datos contiene 549 grabaciones de 290 sujetos (de edades comprendidas entre los 17 y 87 años) de los cuales 52 sujetos corresponden a pacientes sanos. Cada sujeto tiene entre 1 y 5 registros electrocardiográficos de 2 minutos de duración. Cada registro incluye 15 señales medidas simultáneamente: las 12 derivaciones convencionales y las 3 derivaciones de Frank (vx, vy, vz) \cite{PTB_database}. Cabe destacar que esta ultima base de datos fue usada solo con la finalidad de comparar el rendimiento de modelo propuesto en este proyecto con modelos más avanzados propuestos en la literatura, por lo que estos registros no se encuentran presentes en la tabla \ref{tab:distriECGs}.
...
\begin{enumerate}
	...
	\item \textbf{Eliminación del desplazamiento de la línea base:} también conocido como \emph{baseline wander}, es un ruido de baja frecuencia ($<0.5\ Hz$) que distorsiona la morfología del ECG llegando a afectar la detección de las ondas P y T. Para esto se aplica el algoritmo de Sharma \& Sharma \cite{Sharma15} para preservar la morfología real del ECG.
	\item \textbf{Extracción de segmentos ECG:} Debido a que el algoritmo propuesto por Sharma \& Sharma \cite{Sharma15} se desempeña mejor conforme más largo es el segmento a ser tratado por el mismo, se extraen los segmentos de los ECG (de duración fija) luego de haber sido filtrados y que haya sido eliminado el desplazamiento de la línea base. Para homogenizar el análisis, se selecciona una duración fija de 3 minutos, que permitan equilibrar la captura de la variabilidad de los latidos cardíacos. 
...
\begin{enumerate}
	...
	\item La línea base de la señal ECG suele no estar situada en el centro común del papel milimetrado donde queda grabada la señal, es por esto que se debe eliminar el \emph{offset}\footnote{Se refiere al desplazamiento en la señal producto de un voltaje constante propio del instrumento que se suma a la señal que se busca medir} del instrumento y así centrar la línea base del ECG.
\end{enumerate}
...
\end{equation}
esta corresponde con el desplazamiento de la línea base. Por consiguiente, la señal residual $x^{\prime}(t)$ es la combinación de las demás componentes de más bajas amplitudes que resultan ser la señal original sin los artefactos introducidos por el desplazamiento de la línea base y sin cambios en la morfología original de la misma, dicho de otra manera, la señal ECG con la línea base corregida es obtenida sustrayendo el BW de la señal ECG original, esto es:
\begin{equation}
	x^{\prime}(t)=x(t)-x_1(t)
	\label{eq:senal_sin_bw}
\end{equation}
Esta técnica se desempeña mejor conforma la señal sea más prolongada, es por esto que esta remoción de la línea base se hace previo a la segmentación de la señal ECG en intervalos de tiempo más cortos. Como se puede observar en la figura \ref{fig:remocion_bw}, la HVD es capaz de mantener la morfología real del ECG mientras elimina cualquier artefacto de baja frecuencia que haga variar la línea base.
\begin{figure}[h]
	\centering
...
\label{subsec:vector_características}
	...
	\begin{table}[ht]
		...
		\begin{center}
			\begin{tabular}{| M{5cm} | M{8cm} |}
				\hline
				\textbf{Tipos de trastornos clínicos} & \textbf{Descripción} \\ \hline
			...
			\end{tabular}
		\caption{Tipos de trastornos clínicos encontrados.}
...
\begin{enumerate}
	...
	\item \textbf{Clasificación por tipo de trastorno de salud:} Para el modelo de clasificación multiclase por estados de salud, se diseñó una red neuronal con 3 capas densas y una capa de salida con 4 neuronas correspondientes a las 4 categorías de tipos de trastornos clínicos mencionados en la tabla \ref{tab:tipos_trastornos}, la cantidad de neuronas por capa y su función de activación se detallan en la tabla \ref{tab:summary NN clasificacion por trastorno}
	\begin{table}
		\renewcommand{\arraystretch}{1.2}
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
	...
	\begin{table}[ht]
		...
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
				\hline
				\textbf{Capa (tipo)} & \textbf{Número de neuronas} & \textbf{Función activación} \\ \hline
				Densa & 256 & ReLU \\ \hline
				Densa & 128 & ReLU \\ \hline
				Densa & 40 & ReLU \\ \hline
				Densa & 35 & ReLU \\ \hline
				Densa & 25 & ReLU \\ \hline
				Densa & 25 & ReLU \\ \hline
...
%No se hizo una red neuronal para identificar a los pacientes, debido a la baja accuracy que se obtuvo usando tsne
%\subsection{K-means}

\subsection{Análisis comparativo de desempeño usando la base de datos PTB-ECG}

A fin de validar la eficiencia computacional y el desempeño de la cadena de procesamiento propuesta en condiciones análogas a las reportadas en la literatura, se aplicó el esquema completo de preprocesamiento, segmentado de las señales ECG, extracción de características mediante PCA y clasificación con una red neuronal perceptrónica (véase tabla \ref{tab:red_identifica_ptb} para ver la arquitectura de la red propuesta) sobre la base de datos PTB-ECG, empleando una partición 70\%-30\% para entrenamiento y validación, respectivamente. 
...
\begin{table}[ht]
	...
	\begin{center}
		\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
			...
			Densa & 20 & ReLU    \\ \hline
			Densa & 15 & ReLU    \\ \hline
			Densa & 52 & Softmax    \\ \hline
		\end{tabular}
	\end{center}
	\caption{Arquitectura de la red neuronal propuesta para validar y comparar la eficiencia y desempeño de la cadena de procesamiento propuesta.}
	\label{tab:red_identifica_ptb}
\end{table}
...
Cabe destacar que las señales electrocardiográficas que se tienen provienen de la segmentación de las señales originales tomadas de la cantidad total de pacientes reflejados en la tabla \ref{tab:distriECGs}, es por esto que se observará en las siguientes tablas, un aumento significativo de estas señales. Recordemos que cada señal electrocardiográfica fue sometida a un proceso sistemático de extracción de características, conforme se detalla en la subsección \ref{subsec:vector_características}. Como resultado, la representación empleada a partir de este punto corresponde a un vector de características, siendo este el latido promedio de la señal original. Por tanto, en lo sucesivo, cualquier referencia a \guillemetleft señal ECG\guillemetright \ o \guillemetleft señal electrocardiográfica\guillemetright \ implicará su interpretación en términos de dicha representación vectorial, la cual sintetiza la información relevante para las etapas posteriores. Además se establecen tres grupos etarios con el objetivo de analizar el comportamiento dinámico de las señales ECG en función del envejecimiento, esto para las señales electrocardiográficas de los pacientes sanos y patológicos.


\subsection{Distribuciones de los sujetos sanos por sexo y grupos etarios}

Inicialmente se tiene un total de 8540 señales electrocardiográficas provenientes de sujetos sanos, en edades comprendidas entre 18 y 90 años, como se explicó anteriormente, la selección de los grupos etarios permitir distinguir y estudiar las tendencias fisiológicas asociadas a cada etapa de la vida y a su vez esto pudiera facilitar la evaluación comparativa de patrones cardíacos intrínsecos entre los diferentes rangos de edad de la población. En la tabla \ref{tab:Distribucion sanos} se puede apreciar la cantidad electrocardiográficas, edad, distribución por sexo, edad promedio y desviación estándar para cada grupo etario.

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{| M{3cm} | M{2cm} | M{0.8cm} | M{0.8cm} | M{0.8cm} | M{2cm} | M{2.2cm} |}
			\hline
			\textbf{Grupo etario (años)} & \textbf{Cantidad} & \textbf{\%} & \textbf{M} & \textbf{F} & \textbf{Edad promedio} & \textbf{Desviación} \\ \hline 
			(18-34)	& 5131 & 60.0 & 1970 & 3161 & 24.2  & 4.0  \\ \hline
			(35-59)	& 1476 & 17.3 & 752  & 724  & 45.50 & 6.7  \\ \hline
			(> 60)	& 1933 & 22.7 & 694  & 1239 & 73.84 & 7.7  \\ \hline
			\textbf{Total}	& 8540 &      & 3416 & 5124 &		&	   \\ \hline
		\end{tabular}
	\end{center}
	\caption{Distribución de las señales ECG de sujetos sanos según la edad y sexo.}
	\label{tab:Distribucion sanos}
\end{table}

\subsection{Distribuciones de los sujetos patológicos por sexo y grupos etarios}

En este caso se tienen 7391 señales electrocardiográficas de sujetos que guardan alguna cardiopatía, en edades comprendidas entre 22 y 89. En la tabla \ref{tab:Distribucion patologicos} se puede observar la cantidad de señales ECG, edad, distribución por sexo, edad promedio y desviación estándar para cada grupo etario.

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{| M{3cm} | M{2cm} | M{0.9cm} | M{0.8cm} | M{0.8cm} | M{2cm} | M{2.2cm} |}
			\hline
			\textbf{Grupos etario (años)} & \textbf{Cantidad} & \textbf{\%} & \textbf{M} & \textbf{F} & \textbf{Edad promedio} & \textbf{Desviación} \\ \hline 
			(18-34) & 804  & 10.9 & 563  & 241 & 28.8 & 3.3 \\ \hline
			(35-59) & 4143 & 56.1 & 3810 & 333 & 48.4 & 6.8 \\ \hline
			(>60)   & 2445 & 33.0 & 1770 & 675 & 70.8 & 9.2 \\ \hline
			\textbf{Total} & 7391 &  & 6143 & 1249 & & \\ \hline
		\end{tabular}
	\end{center}
	\caption{Distribución de las señales ECG de sujetos patológicos según la edad y sexo.}
	\label{tab:Distribucion patologicos}
\end{table}


\subsection{Distribuciones de los sujetos sanos y patológicos para los entrenamientos de las redes.}

Para garantizar una representación equilibrada de los distintos grupos etarios durante la fase de entrenamiento de las distintas redes neuronales propuestas, se seleccionó aleatoriamente el 60\% de las señales ECG catalogadas como sanos y 60\% de las mismas clasificadas como patológicos dentro de cada grupo. Esta estrategia puede permitir que el modelo aprenda de manera efectiva las diferencias y las trayectorias evolutivas en los perfiles electrocardiográficos. Resulta relevante destacar, como se observa en la tabla \ref{tab:Distribucion entrenamiento}, la tendencia creciente en la prevalencia de patologías a medida que aumenta la edad, lo cual refuerza la importancia de mantener una muestra estratificada para el entrenamiento. Además, la misma tabla expone la distribución final por sexo y los valores promedio de edad de los individuos incluidos en el conjunto de entrenamiento, brindando así información adicional acerca de la representatividad demográfica de la muestra para cada grupo etario.

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{| M{2.7cm} | M{1.75cm} | M{0.65cm} | M{0.75cm} | M{0.7cm} | M{1.1cm} | M{2.2cm} | M{1.8cm} | M{2.1cm} |}
			\hline
			\textbf{Grupos etario (años)} & \textbf{Cantidad} & \textbf{\%} & \textbf{M} & \textbf{F} & \textbf{Sanos} & \textbf{Patológicos} & \textbf{Edad promedio} & \textbf{Desviación} \\ \hline 
			(18-34)  & 3586 & 37.5 & 1540 & 2046 & 3092 & 494  & 25.7 & 4.6 \\ \hline
			(35-59)  & 3330 & 34.8 & 2707 & 623  & 880  & 2450 & 47.3 & 6.7 \\ \hline
			(>60)    & 2648 & 27.7 & 1494 & 1154 & 1197 & 1451 & 72.5 & 7.5 \\ \hline
			\textbf{Total} & 9564 && 5741 & 3823 & 5169 & 4395 &      &     \\ \hline
		\end{tabular}
	\end{center}
	\caption{Distribución de las señales ECG para el entrenamiento de las redes según edad, sexo y estado de salud.}
	\label{tab:Distribucion entrenamiento}
\end{table}


\section{Descripción general de PCA}

La transformación de datos en una representación más adecuada puede facilitar el proceso de aprendizaje de las redes neuronales. Por ejemplo, el uso de un número menor de parámetros, denominados en este caso como componentes principales, para representar la señal en estudio es particularmente importante para fines de reconocimiento y aprendizaje. Dado cualquier conjunto de características para la representación de datos, es importante estimar la dificultad de aprender los conceptos subyacentes utilizando esos datos de entrenamiento \cite{gulerECGbeatClassifier}. Los estudios en la literatura muestran que el grado de dificultad en el entrenamiento de las redes neuronales es inherente al conjunto de ejemplos de entrenamientos dados, es por esto que se hace la canalización del modelo de aprendizaje donde primero se busca por proyectar las señales electrocardiográficas a un espacio de menor dimensionalidad, lo que facilitará posteriores procesos de aprendizajes. Para implementar esta aproximación en este estudio, se establece al análisis de componentes principales como método de reducción de dimensionalidad, paso previo a la fase de entrenamiento de las redes neuronales, donde se busca reducir la dificultad en el entrenamiento mientras se conserva la mayor cantidad de información presente en las señales electrocardiográficas.

Se introducen como datos de entrada para el análisis de componentes principales, los vectores de características mencionados en la subsección \ref{subsec:vector_características} los cuales serán sujetos de la reducción de dimensionalidad.


\subsection{Método de la varianza acumulada}

Para determinar el número óptimo de componentes principales a retener tras la reducción de dimensionalidad de un conjunto de datos usando PCA, se utiliza el método de la varianza acumulada. Cada componente principal representa una dirección de máxima varianza en los datos, y la varianza explicada por cada componente mide la proporción de la información original que dicho componente consigue capturar. La varianza explicada acumulada se obtiene sumando la varianza explicada de los primeros \emph{\textbf{k}} componentes seleccionados, expresada como porcentaje del total. Este valor permite identificar cuántas componentes son suficientes para conservar un porcentaje deseado de la información original (por ejemplo, 90\% o 95\%) y así lograr un equilibrio entre compacidad y capacidad explicativa del modelo resultante. Una forma de proceder muy extendida consiste en seleccionar el número de componentes mínimo a partir del cual el incremento deja de ser sustancial. Como se observa en la figura \ref{fig:varianza_acumulada_explicada}, las primeras 8 componentes principales ya suman una varianza acumulada del $90\%$, por lo que se podrían tomar estas, sin embargo se eligió tomar las primeras 20 componentes principales, que acumulan una varianza del $99\%$, esto debido a que posteriormente se observó mejor desempeño de las redes neuronales ante este aumento de componentes principales de entrada.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\linewidth]{Sem_1/figuras/Varianza_acumulada.png}
	\caption{Gráfico de la varianza acumulada explicada para 20 autovalores.}
	\label{fig:varianza_acumulada_explicada}
\end{figure}

\subsection{Gráficas de dispersión de los registros electrocardiográficos entre componentes principales.}

El gráfico de las primeras dos componentes principales (PC1 vs PC2) (véase fig. \ref{fig:pc1vspc2}) revela un solapamiento considerable entre las distribuciones de los grupos sano (rojo) y patológico (azul), como se aprecia en las curvas de densidad que se observan en el costado izquierdo y región superior de la figura \ref{fig:pc1vspc2}. Aunque ambas poblaciones comparten una región central común, se observa una separabilidad parcial caracterizadas por diferencias en la dispersión espacial, es decir, el grupo sano exhibe una distribución más compacta y centrada, con un pico de densidad claramente definido en valores intermedios de PC1 y ligeramente negativos de PC2 y una distribución bimodal menos pronunciada, con extensiones hacia valores más negativos de PC2 y positivos de PC1. \\

\begin{figure}[h]
	\centering
	\includegraphics[width=0.85\linewidth]{Sem_1/figuras/PC1_vs_PC2_std.png}
	\caption{Gráfico de dispersión de las componentes principales 1 vs 2 de cada registro con distribuciones de densidad marginales.}
	\label{fig:pc1vspc2}
\end{figure}

La proyección de las señales electrocardiográficas sobre la primera y la tercera componente principal (PC1 y PC3) (véase fig. \ref{fig:pc1vspc3}) muestra un patrón de solapamiento significativamente mayor que el observado en el plano PC1-PC2, como se evidencia en las curvas de densidad marginal prácticamente superpuestas para ambos grupos. La distribución bidimensional muestra una nube de puntos altamente entremezclada, donde las poblaciones sanas (rojo) y patológicas (azul)  ocupan regiones espaciales similares, concentrándose principalmente en la región central del plano con extensiones dispersas hacia valores extremos. Las distribuciones marginales confirman esta observación: tanto con PC1 como en PC3, los picos de densidad de ambos grupos coinciden en valores cercanos a cero, con formas y amplitudes muy similares, lo que sugiere que PC3 puede capturar variabilidad intragrupo más que diferencias discriminativas entre estados de salud. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.85\linewidth]{Sem_1/figuras/PC1_vs_PC3_std.png}
	\caption{Gráfico de dispersión de las componentes principales 1 vs 3 de cada registro con distribuciones de densidad marginales.}
	\label{fig:pc1vspc3}
\end{figure}
...
\label{sec:redes_pca}
\subsection{Configuración y métricas de desempeño por modelo}

Para cada una de las redes neuronales desarrolladas, se presentan los parámetros de configuración y las métricas de desempeño obtenidas. Dado que cada modelo fue diseñado para abordar tareas de clasificación con características especificas (clasificación por grupo etario, sexo, estado de salud, tipo de trastornos clínicos e identificación de pacientes), fue necesario ajustar de forma independiente los hiperparámetros clave de cada arquitectura, incluyendo la tasa de aprendizaje (\emph{learning rate}), el número de épocas (\emph{epochs}), el algoritmo de optimización empleado, la función de pérdida utilizada (\emph{loss function}), y la métrica principal de evaluación de desempeño que en este caso, será la exactitud (véase ecuación \ref{eq:exactitud}), cabe destacar que el tamaño del lote (\emph{batch size}) fue el mismo para el entrenamiento de todas las redes, teniendo este un valor de 32. Estos ajustes permiten que cada red alcance el mejor desempeño adaptándose a la complejidad y naturaleza intrínseca de cada problema de clasificación planteado. A continuación se detallan en forma tabular (véase tabla \ref{tab:configuracion y desempeño}) los parámetros específicos configurados para cada modelo, junto con las métricas de desempeño alcanzadas durante las fases de entrenamiento y validación. 

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{| M{2.5cm} | M{2.4cm} | M{1.8cm} | M{2.5cm} | M{2cm} | M{2cm} |}
			\hline 
			\textbf{Tipo de clasificación} & \textbf{Tasa de aprendizaje} & \textbf{Épocas} & \textbf{Optimizador} & \textbf{Función de perdida} & \textbf{Exactitud (\%)} \\ \hline
			Estado de salud & 0.001 & 100 & RMSprop & Entropía binaria cruzada & 96.6 \\ \hline
			Grupos etarios & 0.001 & 100 & RMSprop & Entropía cruzada & 87.1 \\ \hline
			Sexo & 0.001 & 100 & Adam & Entropía binaria cruzada & 98.6 \\ \hline
			Trastorno & 0.001 & 100 & RMSprop & Entropía cruzada & 96.3 \\ \hline
		\end{tabular}
	\end{center}
	\caption{Valores de configuración y desempeño de las redes neuronales propuestas.}
	\label{tab:configuracion y desempeño}
\end{table}


\subsection{Matrices de confusión}

Con el propósito de evaluar visualmente el desempeño de las redes neuronales anteriormente desarrolladas y analizar en detalle los patrones de clasificación obtenidos, se construyeron matrices de confusión para cada una de las tareas implementadas. Dichas matrices permiten cuantificar e identificar qué clases específicas presentan mayor dificultad discriminativas y en qué medida el modelo confunde unas categorías con otras. Este análisis visual permite complementar las métricas numéricas presentadas previamente y facilita el entendimiento del desempeño del clasificador. A continuación, se presentan y analizan las matrices de confusión correspondientes a las tareas de clasificación de estado de salud, tipos de trastorno y grupos etarios.

\subsubsection{Matriz de confusión - Estado de salud (Sanos vs Patológicos).}

La matriz de confusión para la clasificación binaria del estado de salud revela un desempeño excepcional del modelo entrenado sobre las componentes principales. Del total de casos patológicos evaluados, $2840$ fueron correctamente identificados, representando una sensibilidad del $94.5\%$ mientras que unicamente 165 casos ($5.5\%$) fueron erróneamente clasificados como sanos, constituyendo los falsos negativos del sistema. Por otro lado, de los casos sanos, $3255$ fueron correctamente clasificados, alcanzando una especifidad del $96.6\%$, con solo 116 casos ($3.4\%$) incorrectamente clasificados como patológicos.

Estos resultados demuestran que el modelo presenta un balance notable entre sensibilidad y especifidad, con una tasa de acierto superior al $95\%$ en ambas clases. Lo cual puede resultar relevante desde el punto de vista clínico, donde minimizar falsos positivos resulta critico para evitar pasar por alto patologías en pacientes que las presentan. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.80\linewidth]{Sem_1/figuras/Matriz_salud_pca.png}
	\caption{Matriz de confusión para clasificación de estado de salud.}
	\label{fig:mtxsalud}
\end{figure}

\subsubsection{Matriz de confusión - Grupos etarios (Jóvenes, Mediana edad, Mayores).}

La matriz de confusión para la clasificación por grupos etarios  (véase fig. \ref{fig:mtxetarios}), evidencia un desempeño heterogéneo entre las tres categorías, El grupo \guillemetleft Jóvenes\guillemetright alcanza una exactitud del $84.3\%$, con 1985 clasificaciones correctas, aunque exhibe cierta confusión principalmente hacia \guillemetleft Mediana edad\guillemetright (258 clasificaciones incorrectas) y en menor medida hacia \guillemetleft Mayores\guillemetright (113 casos, 4.5\%). El grupo de \guillemetleft Mediana edad\guillemetright presenta un rendimiento de 81.7\% de aciertos (1871 casos) y muestra la mayor tasa de error hacia \guillemetleft Jóvenes\guillemetright con 331 casos (14,5\%). Por su parte, el grupo de \guillemetleft Mayores\guillemetright logra una precisión del 81.2\% (1404 clasificaciones correctas), con errores distribuidos principalmente hacia \guillemetleft Mediana edad\guillemetright (187 casos) y en menor proporción hacia \guillemetleft Jóvenes\guillemetright (139 casos, 8.0\%). Estos resultados evidencian que los patrones electrocardiográficos asociados a la edad presentan una transición gradual en lugar de limites discretos claramente definidos. Las confusiones más frecuentes ocurren entre categorías adyacentes (Jóvenes-Mediana edad y Mediana edad-Mayores), lo cual dificulta la clasificación categórica estricta y refuerza la naturaleza evolutiva de las caracteristicas cardíacas a lo largo del ciclo vital.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.80\linewidth]{Sem_1/figuras/Matriz_etarios_pca.png}
	\caption{Matriz de confusión para clasificación por grupos etarios.}
	\label{fig:mtxetarios}
\end{figure}


\subsubsection{Matriz de confusión - Trastornos de salud (4 clases).}

La matriz de confusión multiclase para la clasificación de trastornos clínicos (mencionados en la tabla \ref{tab:tipos_trastornos}) revela un desempeño diferenciado según la categoría (véase fig. \ref{fig:mtxtrastornos}). La clase \guillemetleft Sanos\guillemetright presenta la mayor capacidad discriminatoria con una precisión del 98.3\% (3314 clasificaciones correctas), exhibiendo tasas de error mínimas (0.4\%, 1.0\% y 0.2\%) distribuidas entre \guillemetleft Sueño\guillemetright, \guillemetleft Ritmo\guillemetright, \guillemetleft Insuficiencia\guillemetright respectivamente. La categoría \guillemetleft Insuficiencia\guillemetright, correspondiente a pacientes con trastornos de la función de bomba del corazón, alcanza un rendimiento notable del 94.2\% con 550 aciertos y confusiones residuales principalmente hacia \guillemetleft Ritmo\guillemetright y \guillemetleft Sanos\guillemetright con 20 y 10 casos respectivamente. Por su parte, la clase \guillemetleft Ritmo\guillemetright, correspondiente a trastornos del ritmo cardíaco, logra una precisión del 93.1\% con 1059 clasificaciones correctas, aunque presenta confusiones moderadas con \guillemetleft Sueño\guillemetright y \guillemetleft Sanos\guillemetright con 2.7\% y 3.3\% de los casos respectivamente. Finalmente, la categoría \guillemetleft Sueño\guillemetright, que agrupa pacientes con trastornos respiratorios durante el sueño, obtiene una precisión del 90.5\% (1162 aciertos), siendo la clase con mayor tasa de error hacia \guillemetleft Sanos\guillemetright, seguida de confusiones menores con \guillemetleft Ritmo\guillemetright e \guillemetleft Insuficiencia\guillemetright con (2.6\% y 0.4\% de los casos). Este patrón sugiere que, si bien el modelo logra separar adecuadamente los casos sanos y las distintas patologías con alta precisión global, la diferenciación entre ciertos subtipos patológicos presenta desafíos adicionales, posiblemente debido a solapamientos en los perfiles electrocardiográficos de las distintas condiciones que se manifiestan clínicamente.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.80\linewidth]{Sem_1/figuras/Matriz_trastornos_pca.png}
	\caption{Matriz de confusión para la clasificación de trastornos clínicos.}
	\label{fig:mtxtrastornos}
\end{figure}

\subsubsection{Matriz de confusión - Sexo (Masculino y femenino)}

La matriz de confusión binaria para la clasificación por sexo revela un desempeño diferenciado (véase fig. \ref{fig:mtxsex}). Se obtuvieron 2368 aciertos en la clase \guillemetleft Masculino\guillemetright que corresponden al 92.8\% del total de pacientes masculinos en la muestra y con una confusión de 183 casos que corresponden al restante 7.2\% de la muestra. En el caso de los pacientes femenino, se obtuvo un 96.8\% de precisión en esta clase que corresponden a 3704 casos, confundiéndose en solo 121 casos (3.2\%). 

Estos resultados demuestran que el modelo es capaz de capturar diferencias electrocardiográficas inherentes al sexo biológico de los pacientes, lo cual tiene relevancia clínica considerable. Las diferencias en las características del ECG entre hombres y mujeres están bien documentadas en la literatura medica. La alta precisión alcanza valida que las componentes principales extraídas capturan efectivamente estas diferencias sutiles pero consistentes, lo que sugiere que el modelo podría emplearse para la clasificación por sexo. Ademas, este hallazgo resalta la importancia de considerar el sexo como variable relevante en el análisis de señales electrocardiográficas, dado que ignorar estas diferencias podría introducir sesgos en los sistemas de clasificación automática y afectar la precisión diagnostica en poblaciones mixtas.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{Sem_1/figuras/matrix_sex_pca.png}
	\caption{Matriz de confusión para la clasificación por sexo.}
	\label{fig:mtxsex}
\end{figure}

\section{Resultados de identificación de pacientes}

\subsection{Métricas de desempeño}

Según se presenta en la tabla \ref{tab:configuracion y desempeño identificar ecgs}, el modelo de identificación de pacientes logró precisiones del 91.1\%, durante la fase de entrenamiento, demostrando una capacidad notable para aprender los patrones discriminativos presentes en las representaciones electrocardiográficas. Durante la fase de validación, el modelo mantuvo un desempeño elevado con una precisión del 88.2\%, lo que sugiere una adecuada capacidad de generalización a datos no vistos durante el entrenamiento. Sin embargo, al evaluar el modelo con el conjunto de datos de producción (datos completamente independientes), la precisión disminuyó significativamente a 67.1\%, evidenciando una degradación en el rendimiento que podría atribuirse a diferencias en la distribución de los datos. 

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{|  M{2.4cm} | M{1.8cm} | M{2.5cm} | M{2cm} |  M{2cm} | M{2cm} | M{2cm} |}
			\hline 
			\textbf{Tasa de aprendizaje} & \textbf{Épocas} & \textbf{Optimizador} & \textbf{Función de perdida} & \textbf{Exactitud durante entrenamiento (\%)} & \textbf{Exactitud durante validación (\%)} & \textbf{Exactitud durante producción (\%)} \\ \hline
			0.0011 & 200 & Adam & Error cuadrático medio & 91.1\% & 88.3\% & 67.1\% \\ \hline
		\end{tabular}
	\end{center}
	\caption{Valores de configuración y métricas de desempeño de la red neuronal encargada de identificar pacientes.}
	\label{tab:configuracion y desempeño identificar ecgs}
\end{table}

\subsection{Histograma de precisiones por paciente}

Con el fin de obtener un gráfico que represente visualmente los resultados de la red neuronal para la identificación de pacientes y debido a que hacer una matriz de más de setecientas clases resulta intratable visualmente, se calcularon los porcentajes de precisión por clase con el fin de poder obtener posteriormente un histograma de frecuencia de dichos porcentajes de precisión. Este histograma (véase fig. \ref{fig:histograma de precisiones identificaciones ecgs}) revela una distribución bimodal fuertemente asimétrica, con una concentración predominante en valores superiores al 95\%. La barra más alta del histograma, con una frecuencia superior a 80 casos, corresponde al intervalo de precisiones superiores al 96\% aproximadamente, lo que indica que la mayoría de las instancias evaluadas alcanzaron niveles prácticamente perfectos. Este patrón sugiere que, para una porción significativa de los datos de validación, el modelo logra una clasificación casi sin errores, demostrando una capacidad robusta de discriminación en condiciones favorables.

Sin embargo, la distribución también presenta una cola extendida hacia valores bajos de precisión, con frecuencias notables distribuidas entre el 10\% y el 80\%. Esto evidencia la presencia de un subconjunto de casos problemáticos donde el modelo experimenta dificultades considerables, posiblemente asociados a individuos con características electrocardiográficas atípicas, señales ruidosas o patrones que se solapan significativamente con otros pacientes. La curva de densidad suavizada (línea roja) confirma esta tendencia bimodal, mostrando un pico principal en el extremo derecho y un segundo modo menos pronunciado en la región intermedia.

Esta variabilidad en el desempeño individual sugiere que existe heterogeneidad en la dificultad de identificación entre diferentes pacientes. Esto puede estar relacionado con factores como la variabilidad temporal de los registros electrocardiográficos de un mismo individuo así como con limitaciones en la cobertura del conjunto de datos de entrenamiento.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Sem_1/figuras/histo_porcen_precisiones.png}
	\caption{Histograma de frecuencia de los porcentajes de la precisión por clase obtenida por la red neuronal para la identificación de pacientes.}
	\label{fig:histograma de precisiones identificaciones ecgs}
\end{figure}

\subsection{Viabilidad de la huella digital cardíaca como rasgo biométrico}

Las señales electrocardiográficas han sido empleadas exitosamente para desempeñar reconocimiento biométrico en un amplio rango de aplicaciones. Sin embargo, los sistemas biométricos basados en señales electrocardiográficas usualmente son menos precisos que las tecnologías basadas en otras características fisiológicas, como las huellas dactilares o el reconocimiento facial. Para incrementar su rendimiento, es necesario estudiar aproximaciones novedosas. En este aspecto, el proceso de reconocimiento biométrico mediante señales electrocardiográficas requiere de la implementación secuencial de dos fases criticas: en primer lugar, la extracción y selección de características relevantes que capturen las particularidades morfológicas del complejo QRS, intervalos temporales y variabilidad de cada individuo, transformando las señales de alta dimensionalidad en vectores de características compactos; posteriormente, se emplea una red neuronal que, mediante aprendizaje supervisado, pueda modelar las relaciones lineales o no lineales existentes entre dichas características permitiendo así la identificación de cada sujeto. Las señales fisiológicas presentan importantes ventajas para aplicaciones biométricas: i) la dificultad de falsificar dichas señales, ii) todos los individuos vivos presentan estas señales de manera inherente, iii) estas señales contienen información acerca de condiciones clínicas o estados fisiológicos, lo cual puede ser provechoso para otras aplicaciones; iv) la adquisición de dichos registros por largos periodos de tiempo no requiere de acciones explicitas de los usuarios, lo que facilita sus aplicaciones por re-autenticaciones periódicas \cite{labati_ecg_biometric_recognition_using_cnn}. 

Entre las técnicas de análisis de señales fisiológicas, los electrocardiogramas destacan por su madurez y difusión. Además, estudios recientes han demostrado que con el reconocimiento biométrico se puede obtener una precisión satisfactoria para diferentes tipos de aplicaciones \cite{Irvine2009_sequent_individual_verification}. Sin embargo, los sistemas biométricos basados en electrocardiogramas tienen que hacer frente a un conjunto de problemas incluyendo: i) variabilidad intraclase alta causada por la frecuencia cardíaca y actividades realizadas, ii) falta de estudios que demuestren la discriminabilidad en datasets grandes; iii) cambios a largo plazo en las características del ECG de una persona.

Tras una visión exhaustiva de la literatura científica relacionada con el reconocimiento biométrico mediante señales electrocardiográficas y diversos modelos de aprendizaje automático, hasta donde es de nuestro conocimiento, el flujo de trabajo propuesto en este estudio no ha sido aplicado previamente para la identificación biométrica de pacientes utilizando la combinación de análisis de componentes principales y redes neuronales perceptrónicas multicapa de la manera especifica en que se implementa en esta investigación. Los resultados obtenidos demuestran que no solo se alcanzan niveles de exactitud comparables con los reportados en trabajos previos que emplean arquitecturas de aprendizaje profundo más complejas como redes neuronales convolucionales (CNN) o redes recurrentes, como las LSTM (memoria larga a corto plazo, por sus siglas en inglés), sino que además se logra la reducción significativa en el costo computacional asociado tanto al entrenamiento como a la inferencia del modelo. Esta eficiencia computacional representa una ventaja importante para aplicaciones en tiempo real o en dispositivos vestibles que son de recursos computacionales limitados, sin comprometer la capacidad discriminativa del sistema biométrico.


\section{Análisis exploratorio usando t-SNE}

Aprovechando la flexibilidad de t-SNE como técnica de reducción de dimensionalidad no lineal, puede utilizarse para explorar y visualizar conjuntos de datos complejos de alta dimensión proyectando la información en espacios bidimensionales o tridimensionales de manera que se preserven tanto las relaciones de vecindad local como la estructura global relevante entre las muestras. Entre las principales ventajas de t-SNE destacan su capacidad para revelar agrupamientos y patrones ocultos que no son detectables mediante métodos lineales como PCA y la preservación de relaciones no lineales intrínsecas en los datos. En esta sección, se presentan visualizaciones bidimensionales del agrupamiento obtenido mediante t-SNE donde se evidencia la formación de cúmulos que pueden representar subpoblaciones diferenciadas y además, la inspección visual de las proyecciones resultantes es fundamental para evaluar empíricamente si los parámetros de t-SNE, como la perplejidad y el número de iteraciones, han sido adecuadamente optimizados para preservar la estructura local y global relevante de los datos en el espacio reducido.

En la figura \ref{fig:tsne1_vs_tsne2} se observa un patrón de agrupamiento más complejo y definido, con agrupaciones densas y bien delimitadas entre ellas, en comparación con mostrado en la figura \ref{fig:pc1vspc2}. Aunque sigue existiendo un solapamiento considerable entre ambas poblaciones, t-SNE consigue segmentar clusters. Las distribuciones marginales encima y a la derecha del diagrama muestran las regiones con alta densidad en valores negativos de tsne2 y cerca del cero en tsne1 para pacientes patológicos, mientras los sanos presentan mayor dispersión en tsne1 y dos modos hacia valores positivos de tsne2, lo que sugiere subpoblaciones específicas dentro de cada grupo. Es de mencionar que se usaron como datos de entrada de t-SNE, las primeras 40 componentes principales obtenidas de paso anterior, en la tabla \ref{tab:valores_tsne} se reflejan los demás valores de configuración de t-SNE. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.85\linewidth]{Sem_1/figuras/tsne1_vs_tsne2.png}
	\caption{Gráfico de dispersión entre tsne1 y tsne2.}
	\label{fig:tsne1_vs_tsne2}
\end{figure}
...
\begin{table}[ht]
	...
	\begin{center}
		\begin{tabular}{| M{2.5cm} | M{2.5cm} | M{2.5cm} | M{2.5cm} |}
			\hline
			\textbf{N° componentes de entrada} & \textbf{Perplejidad} & \textbf{N° de iteraciones} & \textbf{N° componentes de salida} \\ \hline
			40 & 25 & 1000 & 3 \\ \hline
		\end{tabular}
	\end{center}
	\caption{Valores de configuración de t-SNE.}
	\label{tab:valores_tsne}
\end{table}

t-SNE se ha vuelvo ampliamente popular debido a su casi mágica habilidad para crear \guillemetleft mapas\guillemetright a partir de la data que puede tener cientos o miles de dimensiones. Aunque impresionante, estas imágenes pueden llevar a interpretaciones erróneas, es por esto que resulta necesario usar otras herramientas para interpretar estos \guillemetleft mapas\guillemetright


\section{Resultados de clasificación usando los datos de tsne}

Con el objetivo de profundizar en la interpretación de las representaciones generadas por t-SNE, se utilizarán redes neuronales perceptrónicas como modelos traductores entre las componentes no lineales de t-SNE y las variables clínicas de interés. Esta aproximación habilita la extracción de inferencias clínicas relevantes a partir de la estructura latente descubierta por t-SNE en los datos originales.

\subsection{Configuración y métricas de desempeño por modelo}

De manera similar que en la sección \ref{sec:redes_pca}, la siguiente subsección especifica los valores de configuración de cada red neuronal usada (véase tabla \ref{tab:configuracion y desempeño traductor tsne}) para extraer inferencias clínicas relevantes a partir de la estructura latente de las representaciones generadas por t-SNE. Es importante destacar que en este caso se usó un 50\% de la data para entrenamiento y el resto para producción.

\begin{table}[ht]
	\renewcommand{\arraystretch}{1.2}
	\begin{center}
		\begin{tabular}{| M{2.5cm} | M{2.4cm} | M{1.8cm} | M{2.5cm} | M{2cm} | M{1.8cm} |}
		\hline 
		\textbf{Tipo de clasificación} & \textbf{Tasa de aprendizaje} & \textbf{Épocas} & \textbf{Optimizador} & \textbf{Función de perdida} & \textbf{Exactitud (\%)} \\ \hline
			Estado de salud & 0.001 & 300 & Adam & Entropía binaria cruzada & 98.6\%  \\ \hline
			Grupos etarios  & 0.001 & 300 & Adam & Entropía cruzada & 95.5\%  \\ \hline
			Sexo            & 0.001 & 300 & Adam & Entropía binaria cruzada & 96.3\%  \\ \hline
			Trastorno       & 0.001 & 300 & Adam & Entropía cruzada & 96.8\%  \\ \hline
		\end{tabular}
	\end{center}
	\caption{Valores de configuración y métricas de desempeño de las redes neuronales encargadas de interpretar los datos de t-SNE.}
	\label{tab:configuracion y desempeño traductor tsne}
\end{table}

\subsection{Matrices de confusión}

\subsubsection{Matriz de confusión - Estado de salud}

La matriz de confusión para la clasificación binaria del estado de salud en el espacio de t-SNE revela un desempeño excepcional y superior al obtenido en el espacio de PCA, como se observa en la figura \ref{fig:mtxsaludtsne}. La clase \guillemetleft Sano\guillemetright alcanza una precisión del 99.6\% (4199 identificaciones correctas), con unicamente 17 casos (0.4\%) erróneamente clasificados como patológicos, lo que representa una tasa de falsos positivos extremadamente baja. Por su parte, la clase \guillemetleft Patológico\guillemetright presenta una precisión del 97.4\% (3658 aciertos), con 96 casos incorrectamente (2.6\%) clasificados como sanos, constituyendo los falsos negativos del sistema. 

Estos resultados demuestran que la representación no lineal proporcionada por t-SNE captura de manera más efectiva las diferencias estructurales entre los estados de salud en comparación con el espacio lineal de PCA. La reducción significativa en la tasa de falsos negativos (de 5.5\% en PCA a 2.6\% en t-SNE) es particularmente relevante desde el punto de vista clínico, ya que disminuye el riesgo de pasar por alto pacientes con condiciones patológicas. Asimismo, la especifidad del 99.6\% confirma que prácticamente todos los individuos sanos son correctamente identificados, minimizando intervenciones innecesarias. Este desempeño superior valida la hipótesis de que t-SNE preserva relaciones locales y no lineales críticas para la discriminación entre estados de salud, superando las limitaciones de los métodos lineales tradicionales en tareas de clasificación biomédica compleja. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{Sem_1/figuras/Matriz_salud_tsne.png}
	\caption{Matriz de confusión para la clasificación por estado de salud en el espacio de t-SNE.}
	\label{fig:mtxsaludtsne}
\end{figure}

\subsubsection{Matriz de confusión - Grupos etarios}

La matriz de confusión para la clasificación por grupos etarios en el espacio de t-SNE muestra un desempeño notablemente mejorado respecto al espacio PCA (véase fig. \ref{fig:mtxetariostsne}). El grupo \guillemetleft Mayores\guillemetright exhibe la mejor precisión con un 97.0\% (2110 clasificaciones correctas), presentando errores mínimos hacia \guillemetleft Jóvenes\guillemetright y \guillemetleft Mediana edad\guillemetright con 21 y 40 casos incorrectamente identificados. El grupo \guillemetleft Mediana edad\guillemetright alcanza una precisión del 95.4\% (2706 aciertos), con confusiones principalmente hacia \guillemetleft Jóvenes\guillemetright (70 casos, 2.5\%) y en menor medida hacia \guillemetleft Mayores\guillemetright (60 casos, 2.1\%). Por su parte, el grupo \guillemetleft Jóvenes\guillemetright logra una precisión del 94.5\% (2797 identificaciones correctas), mostrando la mayor tasa relativa de confusión hacia \guillemetleft Mediana edad\guillemetright (117 casos 4,0\%) y confusiones menores hacia \guillemetleft Mayores\guillemetright (45 casos, 1.5\%).

La mejora sustancia mostrada en los resultados antes mencionados en comparación con PCA, donde los resultados rondaban el 81\%, elevándose ahora a un rango del 94.5\%-97\% demuestra la mejora significativa que confirma que la representación no lineal de t-SNE captura de manera más efectiva las transiciones graduales en los patrones electrocardiográficos asociados a la edad. Aunque persiste cierta confusión entre grupos contiguos (especialmente Jóvenes-Mediana Edad), las tasas de error se han reducido considerablemente, validando la capacidad de t-SNE para resolver mejor las fronteras difusas entre categorías etarias adyacentes y reforzando su utilidad para aplicaciones de clasificación que requieren discriminación de patrones evolutivos continuos en señales biomédicas.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Sem_1/figuras/Matriz_etarios_tsne.png}
	\caption{Matriz de confusión para clasificación por grupos etarios en el espacio de t-SNE.}
	\label{fig:mtxetariostsne}
\end{figure}

\subsubsection{Matriz de confusión - Trastorno de salud}

Al igual que con las matrices de confusión anteriores, la matriz de confusión multiclase para la clasificación de trastornos clínicos en el espacio de t-SNE evidencia un desempeño extraordinario y sustancialmente mejorado respecto al espacio PCA (véase fig. \ref{fig:mtxtrastornotsne}). La clase \guillemetleft Sanos\guillemetright alcanza la mayor precisión con un 98.5\% (4152 clasificaciones correctas), exhibiendo tasas de error mínimas distribuidas entre \guillemetleft Sueño\guillemetright (0.7\%, 31 casos), \guillemetleft Ritmo\guillemetright (0.5\%, 22 casos) e \guillemetleft Insuficiencia\guillemetright (0.3\%, 11 casos). La categoría \guillemetleft Sueño\guillemetright, correspondiente a trastornos respiratorios durante el sueño, logra una precisión del 95.1\% (1534 aciertos), con confusiones principalmente hacia \guillemetleft Sanos\guillemetright (3.2\%, 51 casos), \guillemetleft Insuficiencia\guillemetright (1.5\%, 24 casos) y prácticamente ninguna hacia \guillemetleft Ritmo\guillemetright (0.2\%, 4 casos).

El grupo \guillemetleft Ritmo\guillemetright asociado a trastornos del ritmo cardíaco, presenta una precisión del 94.8\% (1344 clasificaciones correctas), mostrando confusiones moderadas hacia \guillemetleft Sanos\guillemetright (2.5\%, 36 casos), \guillemetleft Sueño\guillemetright (1.5\%, 21 casos) e \guillemetleft Insuficiencia\guillemetright (1.1\%, 16 casos). Finalmente, la categoría \guillemetleft Insuficiencia\guillemetright, correspondiente a trastornos de la función de bomba del corazón, obtiene una precisión del 94.5\% (684 aciertos), con errores distribuidos principalmente hacia \guillemetleft Sanos\guillemetright con 3.0\% de error y 22 casos incorrectamente identificados, \guillemetleft Sueño\guillemetright (2.3\% y 17 casos) y mínimamente hacia \guillemetleft Ritmo\guillemetright (0.1\%, 1 caso).

Comparando con PCA (donde las precisiones variaban entre 90.5\%-98.3\%), t-SNE mantiene e incluso mejora ligeramente el desempeño en algunas categorías, alcanzando un rango de 94.8\%-98.5\%. Este resultado confirma que la representación no lineal de t-SNE preserva eficazmente las sutilezas discriminativas entre subtipos patológicos con perfiles electrocardiográficos potencialmente solapados, lo que valida su aplicabilidad en tareas complejas de diagnostico diferencial automatizado.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Sem_1/figuras/Matriz_trastornos_tsne.png}
	\caption{Matriz de confusión para la clasificación de trastornos clínicos en el espacio de t-SNE.}
	\label{fig:mtxtrastornotsne}
\end{figure}

\subsubsection{Matriz de confusión - Sexo}

La matriz de confusión binaria para la clasificación por sexo en el espacio de t-SNE revela un desempeño robusto, aunque ahora con resultados mixtos en comparación con PCA (véase fig. \ref{fig:mtxsextsne}). La clase \guillemetleft Masculino\guillemetright alcanza una precisión del 98.0\% (4724 clasificaciones correctas), con unicamente 98 casos (2.0\%) erróneamente clasificados como femeninos, lo que representa una mejora sustancial frente al 92.8\% obtenido en PCA. Sin embargo, la clase \guillemetleft Femenino\guillemetright presenta una precisión del 93.9\% (2955 casos correctamente identificados), confundiéndose en 193 casos (6.1\%) como masculinos, lo cual representa una ligera disminución respecto al 96.8\% alcanzado en PCA.

Este patrón asimétrico sugiere que, mientras t-SNE mejora significativamente la identificación de pacientes masculinos al reducir los falsos positivos a la mitad (de 7.2\% a 2.0\%), la representación no lineal introduce mayor variabilidad en la clase femenina, casi duplicando su tasa de error (de 3.2\% a 6.1\%). Esta diferencia podría atribuirse a que t-SNE, al optimizar la preservación de vecindarios locales, captura de manera más efectiva las características distintivas de los patrones electrocardiográficos masculinos, pero simultáneamente introduce mayor dispersión en el subespacio correspondiente a las muestras femeninas. A pesar de esta asimetría, el modelo mantiene una exactitud global superior al 96\% (véase tabla \ref{tab:configuracion y desempeño traductor tsne}), validando que ambas técnicas de reducción de dimensionalidad capturan efectivamente las diferencias electrocardiográficas inherentes al sexo biológico.\\

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Sem_1/figuras/matriz_sex_tsne.png}
	\caption{Matriz de confusión para la clasificación por sexo en el espacio de t-SNE.}
	\label{fig:mtxsextsne}
\end{figure}


En conjunto, los resultados obtenidos mediante la clasificación en el espacio de t-SNE demuestran la eficacia superior de las representaciones no lineales para la mayoría de las tareas evaluadas. En comparación con PCA, t-SNE logra mejoras sustanciales en la clasificación de estado de salud (incremento de 2.9 puntos porcentuales en precisión para la clase patológica y 3.0 puntos en precisión para sanos), así como mejoras notables en la discriminación de grupos etarios (incrementos de 10 a a casi 16 puntos porcentuales en las tres categorías). Para la clasificación de tipos de trastorno, t-SNE mantiene un desempeño ligeramente superior al de PCA, consolidando precisiones superiores al 94\% en todas las categorías patológicas. La clasificación por sexo presenta resultados mixtos, con mejora significativa para la clase masculina pero ligera reducción en la femenina, sugiriendo que la preservación de vecindarios locales favorece asimétricamente ciertas características discriminativas. Estos hallazgos validan la hipótesis de que t-SNE captura relaciones no lineales críticas en los datos electrocardiográficos que las proyecciones lineales no pueden resolver completamente, reforzando su utilidad como herramienta complementaria en el análisis exploratorio y clasificación de señales biomédicas complejas.

\subsection{Dendrogramas}

Con el propósito de corroborar cuantitativamente las diferencias inherentes entre los perfiles electrocardiográficos de pacientes sanos y patológicos, se empleó la distancia de Mahalanobis sobre las representaciones obtenidas en el espacio no lineal de t-SNE. Para analizar la estructura jerárquica entre estos perfiles, se construyó un dendrograma a partir de un esquema de agrupamiento jerárquico utilizando dicha distancia como métrica de similitud. Dado el elevado costo computacional asociado al calculo de la matriz completa de distancias pareadas, el análisis se realizó sobre una muestra aleatoria representativa correspondiente al 10\% de las representaciones electrocardiográficas de los pacientes en el espacio de t-SNE, preservando así la estructura multivariada relevante y permitiendo visualizar de forma clara las agrupaciones entre estados de salud.

El dendrograma (véase fig. \ref{fig:dendro_10_percent}) muestra una bifurcación principal definida en la línea de clasificación ($sigma=7.621$) que corresponde a la línea verde punteada en la figura \ref{fig:dendro_10_percent}, que divide claramente a los pacientes en dos grandes grupos: pacientes sanos (naranjas, parte superior) y pacientes patológicos (azul, parte inferior). Esta separación temprana y a gran distancia indica que las características electrocardiográficas entre ambos estados de salud son diferenciadas en el espacio multivariado. Además se observa que los pacientes sanos forman un clúster muy compacto y homogéneo, con fusiones que ocurren predominantes a distancia bajas (menores a 6), lo que sugiere alta similitud entre los perfiles electrocardiográficos de individuos sin patologías. Por el contrario, los pacientes patológicos presentan mayor heterogeneidad interna, con sub-agrupamientos que se fusionan a distancias variables, reflejando la diversidad de trastornos cardíacos presentes en este grupo.

\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\linewidth]{Sem_1/figuras/Mahala_10_percent.png}
	\caption{Dendrograma de clasificación con etiquetas de estado de salud del 10\% de los pacientes.}
	\label{fig:dendro_10_percent}
\end{figure}

A fin de corroborar la reproducibilidad de la estructura jerárquica detectada, se construyó un segundo dendrograma basado en el 30\% de los datos, permitiendo evaluar la estabilidad de los clusters identificados ante variaciones en el tamaño de la muestra analizada. 

El dendrograma mostrado en la figura \ref{fig:dendro_30_percent} presenta características estructurales muy similares al dendrograma del 10\%, este dendrograma de la figura \ref{fig:dendro_30_percent} revela tres clusters principales claramente diferenciados por colores: rojo (inferior), morado (medio) y azul (superior). La primera bifurcación crítica ocurre en $sigma=7.621$ (línea punteada negra de la izquierda) dicho valor es idéntico al de la línea de clasificación del dendrograma de la figura \ref{fig:dendro_10_percent}, este valor de $sigma$ parece separar nuevamente los pacientes patológicos de los pacientes sanos. Una segunda bifurcación importante (línea punteada de la derecha, $sigma=4.55$) subdivide a el grupo superior en dos subclusters (morado y azul), sugiriendo una estructura jerárquica más compleja con mayor resolución. 

Esta distribución tricolor delimita tres regiones en el dendrograma, dentro de las cuales la distancia de Mahalanobis aplicada a las representaciones electrocardiográficas en el espacio de t-SNE permite clasificar a los pacientes en estados de salud sanos y patológicos con altos niveles de precisión. En cada una de estas zonas es posible cuantificar el rendimiento del esquema de agrupamiento, obteniéndose porcentajes de acierto específicos por clase que reflejan cómo varía la capacidad discriminativa del método a lo largo del eje de distancias. 

En la primera zona (clúster rojo) se alcanza una precisión de 30\% para la clase \guillemetleft Sano\guillemetright y de 70\% para la clase \guillemetleft Patológico\guillemetright; en la segunda zona (clúster morado), se obtiene una precisión del 69.4\% para la clase \guillemetleft Sano\guillemetright y un 30.6\% para la clase \guillemetleft Patológico\guillemetright; y en la tercera zona (clúster azul) se consigue una precisión para la clase \guillemetleft Sano\guillemetright del 95.3\% y para la clase patológico del 4.7\% restante. 

\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{Sem_1/figuras/Mahala_30_percent.png}
	\caption{Dendrograma de clasificación con etiquetas de estado de salud del 30\% de los pacientes.}
	\label{fig:dendro_30_percent}
\end{figure}

En conjunto, estos resultados sugieren que, aun utilizando exclusivamente la distancia de Mahalanobis sobre un subconjunto de pacientes, es posible estimar con las precisiones previamente reportadas la probabilidad de que las representaciones electrocardiográficas en el espacio de t-SNE correspondan a individuos sanos o con algún tipo de patología. Esto pone de manifiesto el poder discriminativo de la métrica en el espacio reducido; sin embargo, también evidencia que dicho enfoque sigue siendo lineal en el sentido de la frontera de decisión y, por tanto, no explota completamente la estructura no lineal del espacio latente. En este contexto, el uso de redes neuronales resulta especialmente ventajoso, ya que permite aprender de manera automática características más sutiles y patrones latentes dentro del espacio de t-SNE, mejorando la capacidad del sistema para distinguir con mayor precisión el estado de salud asociado a cada electrocardiograma.

\subsection{Análisis comparativo de desempeño usando PTB-ECG}

Los resultados evidencian una mejora sustancial en coste computacional y una ligera reducción en exactitud: mientras que el tiempo de entrenamiento se redujo de aproximadamente 9 horas a solo 13 segundos, la exactitud del modelo bajó al 96\%. En comparación a los valores reportados en trabajos previos, estos obtienen exactitudes en el rango entre el 97\% al 100\% (véase tabla \ref{tab:comparacion_ptb}). Estos hallazgos confirman que la combinación de técnicas de preprocesamiento robusto, reducción de dimensionalidad lineal mediante PCA y arquitecturas neuronales simples constituye una alternativa eficiente para identificación de registros electrocardiográficos en comparación con métodos más complejos o que operan directamente sobre señales crudas de alta dimensión. Cabe mencionar que la red propuesta anteriormente tiene una tasa de aprendizaje de $0.001$, fue entrenada con $80$ épocas y usando el error cuadrático medio como función de perdida.
...
\begin{table}[ht]
	...
	\begin{center}
		\begin{tabular}{| M{2.5cm} | M{3cm} | M{3.5cm} |}
			\hline
			\textbf{Método} & \textbf{Exactitud (\%)} & \textbf{Tiempo de entrenamiento} \\ \hline
			Deep-ECG\cite{labati_ecg_biometric_recognition_using_cnn} & 100 & 9h 22m \\ \hline
			Propio & 96 & 12 s \\ \hline
		\end{tabular}
	\end{center}
	\caption{Valores de exactitud y tiempo de entrenamiento entre diferentes métodos para la base de datos PTB-ECG.}
	\label{tab:comparacion_ptb}
\end{table}
...
La implementación conjunta del algoritmo clásico de detección de complejos QRS de Pan \& Tompkins y la descomposición vibracional de Hilbert para la corrección del desplazamiento de la línea base constituye un esquema de preprocesamiento robusto y eficiente para señales electrocardiográficas. En combinación, estos métodos permiten atenuar de forma efectiva ruido de alta y baja frecuencia, así como artefactos de movimiento, generando registros electrocardiográficos depurados	para la posterior extracción de características mediante el análisis de componentes principales y t-SNE. \\

Los modelos de redes neuronales perceptrónicas propuestas para la identificación de pacientes y clasificación según variables clínicas alcanzaron exactitudes superiores al 85\% cuando se entrenaron con representaciones en el espacio de PCA y superiores al 95\% cuando se utilizaron las incrustaciones obtenidas con t-SNE. Estos resultados evidencian que las técnicas de reducción de dimensionalidad empleadas logran concentrar información discriminativa relevante en un espacio de menor dimensión, optimizando el costo computacional y facilitando que los modelos de aprendizaje automático aprendas relaciones no triviales entre los patrones electrocardiográficos y las variables clínicas de interés. \\

En conjunto, la cadena de procesamiento propuesta ---basada en un preprocesamiento robusto del ECG, reducción de dimensionalidad mediante PCA y t-SNE y posterior modelado con redes neuronales--- demuestra ser una estrategia efectiva para abordar los objetivos específicos del estudio. El preprocesamiento garantiza señales depuradas, las proyecciones en espacios reducidos capturan las características más informativas de los perfiles electrocardiográficos y los modelos predictivos alcanzan niveles de exactitud elevados tanto en tareas de identificación como de clasificación clínica, respaldando la viabilidad del ECG como marcador biométrico. \\

En síntesis, los resultados obtenidos con la base de datos PTB-ECG muestran que la cadena de procesamiento propuesta permite alcanzar exactitudes comparables o superiores a las reportadas en la literatura, pero con un coste computacional drásticamente menor (pasando de escalas del orden de horas a solo unos segundos de entrenamiento y validación). Asimismo, hasta donde es de nuestro conocimiento, no se han encontrado trabajos que apliquen exactamente esta combinación secuencia de técnicas para la tarea de identificación de electrocardiogramas, por lo que el esquema aquí planteado constituye una primera aproximación metodológica. Este hecho resulta especialmente relevante, ya que demuestra que es posible obtener desempeños cercanos al 100\% de exactitud utilizando modelos relativamente simples y ligeros, lo que abre la puerta a implementaciones clínicas y biométricas más accesibles, reproducibles y eficientes en recursos, sin sacrificar la calidad de las decisiones diagnosticas o de identificación.

\section{Recomendaciones}

Se recomienda extender la base de datos incorporando un numero significativamente mayor de registros electrocardiográficos de sujetos de control y pacientes con diversas patologías cardiovasculares provenientes de la población venezolana. Esto permitiría evaluar el efecto del contexto geográfico, socioeconómico y étnico sobre la morfología del ECG y sobre la distribución de las representaciones en los espacios reducidos de PCA y t-SNE, lo que podría manifestarse en la aparición de nuevos clústeres o en desplazamiento de los ya observados. Una base de datos más amplia y representativa contribuiría a reducir el sesgo de muestreo e incrementaría la capacidad de generalización de los modelos.\\

Resulta aconsejable utilizar bases de datos que incluyan diagnósticos clínicos más detallados y estandarizados, idealmente anotados por múltiples especialistas y respaldados por criterios diagnósticos consensuados. La incorporación de etiquetas mas finas permitiría entrenar modelos multiclase o jerárquicos capaces de distinguir no solo entre estado sano y patológico, sino entre fenotipos patológicos concretos. Además, la existencia de anotaciones interobservador posibilitaría estudiar la concordancia diagnostica y cuantificar el potencial de las redes neuronales para reducir la variabilidad entre expertos.\\

Se recomienda ampliar el análisis incorporando, además de la derivación II, un conjunto mayor de derivaciones (por ejemplo, las 12 derivaciones convencionales) o, al menos, un subconjunto representativo de derivaciones precordiales y de miembros. El uso de múltiples canales permitiría capturar de forma más completa la distribución espacial de la actividad eléctrica cardíaca, mejorando la sensibilidad a fenómenos localizados como isquemias regionales, bloqueos de rama o hipertrofias ventriculares. Desde un punto de vista metodológico, esto abriría la puerta al empleo de modelos que exploten explícitamente la estructura multicanal, lo que potencialmente incrementaría la capacidad discriminativa tanto para clasificación clínica como para identificación biométrica basada en ECG.
...
Se enfatiza que la interpretación y uso de los modelos automáticos debe estar siempre supervisada por expertos clínicos, reconociendo que las decisiones medicas requieres una evaluación integral que los algoritmos no pueden sustituir, sino complementar de manera responsable.
