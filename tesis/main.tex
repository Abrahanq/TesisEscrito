...
\begin{table}[ht]
	...
	\begin{center}
		\begin{tabular}{|p{1.85cm}|M{1.7cm}|M{1.55cm}|M{1.15cm}|M{2.2cm}|M{1.5cm}|M{1.5cm}|M{1.5cm}|M{1cm}|}
			...
			\hline
			\textbf{Cantidad de ECGs} & \centering352 & 410 & 611 & 151 & 435 & 222 & 106 & 762 \\
			\hline
		\end{tabular}
...
Tal como se discutió previamente, la naturaleza y complejidad de cada problema de identificación clínica exige un enfoque particular en torno a la construcción de las redes neuronales empleadas. En esta subsección, se detalla exclusivamente la estructura y características técnicas de las redes configuradas para cada tarea, considerando los retos que plantea la predicción de las diferentes variables clínicas consideradas en este trabajo. De este modo, se busca mostrar cómo la adaptación de la arquitectura influye directamente en la capacidad de la red para traducir las representaciones obtenidas de t-SNE.


\begin{enumerate}
	\item \textbf{Clasificación por estado de salud:} 
	\begin{table}[ht]
		\renewcommand{\arraystretch}{1.2}
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
			...
			\end{tabular}
		\end{center}
		\caption{Arquitectura de la red neuronal para la clasificación por estado de salud con los datos obtenidos con t-SNE.}
		\label{tab:summary NN estado de salud_tsne}
	\end{table}

	\item \textbf{Clasificación por grupos etarios:}
	\begin{table}[ht]
		\renewcommand{\arraystretch}{1.2}
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
		...
		\end{center}
		\caption{Arquitectura de la red neuronal para la clasificación por estado de salud con los datos obtenidos con t-SNE.}
		\label{tab:summary NN grupos etarios_tsne}
	\end{table}

	\item \textbf{Clasificación por sexo:}
		\begin{table}[ht]
		\renewcommand{\arraystretch}{1.2}
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
				...
				Densa & 20 & ReLU    \\ \hline
				Densa & 20 & ReLU    \\ \hline
				Densa & 1  & Sigmoide    \\ \hline
			\end{tabular}
		\end{center}
		\caption{Arquitectura de la red neuronal para la clasificación por sexo con los datos obtenidos con t-SNE.}
		\label{tab:summary NN sex_tsne}
	\end{table}

	\item \textbf{Clasificación por tipo de trastorno de salud:}
		\begin{table}[ht]
		\renewcommand{\arraystretch}{1.2}
		\begin{center}
			\begin{tabular}{| M{2.5cm} | M{3cm} | M{3cm} |}
			...
			\end{tabular}
		\end{center}
		\caption{Arquitectura de la red neuronal para la clasificación por tipo de trastorno de salud con los datos obtenidos con t-SNE.}
		\label{tab:summary NN trastornos_tsne}
	\end{table}
...
Con el fin de validar los resultados salientes de las redes neuronales de manera objetiva y sistemática, durante el proceso de entrenamiento de las redes neuronales, se dividió el conjunto de datos para entrenamiento y validación, en 50\% para cada subconju%En esta subsección, describo los procedimientos aplicados para asegurar que los modelos desarrollados (redes neuronales, t-SNE + redes, etc.) se evalúan de manera objetiva y sistemática. Normalmente esto incluye la división de conjuntos de datos en entrenamiento, validación y prueba, el uso de métricas como exactitud, sensibilidad, especificidad, o F1-score, y la aplicación de técnicas como validación cruzada para evaluar la generalización de los modelos.
\subsection{clasificacion}
%cómo se evaluó la capacidad de los modelos para clasificar correctamente las señales o patrones de ECG en sus respectivas categorías (por ejemplo, tipos de arritmia, latidos normales, etc.). Aquí discute los resultados obtenidos en términos de precisión, matriz de confusión y otras métricas relevantes, comparando si es pertinente los distintos métodos o representaciones de entrada (PCA vs t-SNE)
\subsection{identificacion}
%Explora el desempeño en tareas que impliquen identificar o distinguir instancias individuales, como pacientes, segmentos, o ritmos específicos. Puedes resaltar la importancia de la sensibilidad y la capacidad del modelo para discriminar entre subgrupos.
\subsection{repetibilidad}
%Aquí detalla los mecanismos o experimentos que corroboran que los resultados del modelo sean consistentes cuando se repiten bajo las mismas condiciones. Esto puede implicar repetir corridas con diferentes semillas aleatorias, evaluar la variabilidad de los resultados y documentar la estabilidad de los procesos de entrenamiento y las salidas de t-SNE.
%\subsection{analisis de robustez}Explica los análisis diseñados para probar el comportamiento de los modelos ante variaciones, ruido o datos atípicos. Esto puede incluir pruebas de sensibilidad a cambios en los parámetros, inclusión de ruido sintético en los datos, o comparación de desempeño en subconjuntos diversos.
\subsection{consideraciones éticas}
%Reconoce implicaciones éticas relacionadas con el manejo de datos de pacientes, la privacidad, el consentimiento y la correcta interpretación y uso de los resultados de modelos automáticos en contextos médicos.
